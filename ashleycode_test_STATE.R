...
data_tbl_total$GEOID <- with(data_tbl_total, paste0(state_fips_code))
data_tbl_dtc$GEOID <- with(data_tbl_dtc, paste0(state_fips_code))
...
re_sf <- state_race %>% left_join(., data_red, by = "GEOID" ) %>%
...
####### FRUIT ACRES BEARING (NON CITRUS) ---------------------------
...
nass_api_url_dtc$query <- list(source_desc                 = 'CENSUS',
                               ...
                               reference_period_desc       = 'YEAR' ,
                               year                        = '2022',
                               short_desc                  = 'NON-CITRUS TOTALS, (EXCL BERRIES) - ACRES BEARING & NON-BEARING' ,
                               domain_desc                 = 'TOTAL' ,
                               agg_level_desc              = 'STATE',
                               key                         = my_api_token)
...
# Create a color palette for the breaks
colors <- c("#D3D3D3", "#F5C6C6","#F28E8E","#EB5656", "#E02E2E")

# Create the map
intexpmap<- tm_shape(md_base) +
  tm_borders(lwd = 0.4, col = "grey60") +  # Add county borders
  tm_fill(col = "value", style = "fixed", breaks = breaks, palette = colors,
          title = "Percent of Ag Land in Non-Citrus Fruits", labels = labels,
          textNA = "Missing data", colorNA = "#E8E8E8") +
  tm_layout(legend.outside = TRUE, legend.position = c("right", "top"),
            legend.title.size = 0.8, legend.text.size = 0.7,
            legend.format = list(fun = function(x) formatC(x, format = "d"))) +
  tm_basemap(server = "OpenStreetMap")  # Choose a basemap (you can customize this)
...
nass_api_url_dtc$query <- list(source_desc                 = 'CENSUS',
                               ...
                               reference_period_desc       = 'YEAR' ,
                               year                        = '2022',
                               short_desc                  = 'CITRUS TOTALS - ACRES BEARING & NON-BEARING' ,
                               domain_desc                 = 'TOTAL' ,
                               agg_level_desc              = 'STATE',
                               key                         = my_api_token)
...
# Create a color palette for the breaks
colors <- c("#D3D3D3", "#F6E3C5","#F4C589","#F29A4A", "#D9752D")

# Create the map
intexpmap<- tm_shape(md_base) +
  tm_borders(lwd = 0.4, col = "grey60") +  # Add county borders
  tm_fill(col = "value", style = "fixed", breaks = breaks, palette = colors,
          title = "Percent of Ag Land in Citrus Fruits", labels = labels,
          textNA = "Missing data", colorNA = "#E8E8E8") +
  tm_layout(legend.outside = TRUE, legend.position = c("right", "top"),
            legend.title.size = 0.8, legend.text.size = 0.7,
            legend.format = list(fun = function(x) formatC(x, format = "d"))) +
